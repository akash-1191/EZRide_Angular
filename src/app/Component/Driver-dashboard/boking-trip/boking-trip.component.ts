...
@Component({
  selector: 'app-boking-trip',
  imports: [FormsModule, CommonModule],
  templateUrl: './boking-trip.component.html',
  styleUrl: './boking-trip.component.css'
})
export class BokingTripComponent implements OnInit {
  ...
  getBikeTripsCount(): number {
    ...
    t => t.status === 'Assigned' && t.vehicleType === 'Bike'
  ).length;
  }
  ...
  getCarTripsCount(): number {
    return this.trips.filter(
    t => t.status === 'Assigned' && t.vehicleType === 'Car'
  ).length;
  }

  getTotalTripsCount(): number {
  return this.trips.filter(t => t.status === 'Assigned').length;
}
  ...
  applyFilters(): void {
    let filtered = this.trips.filter(
  trip => trip.status === 'Assigned'
);
    // Search filter
  ...
  updateStatusActive(driverBookingId: number): void {
    console.log('Driver Booking ID:', driverBookingId);
    this.driverbookingidis = driverBookingId;
    this.services.driverUpdateStatusBusy(this.driverbookingidis).subscribe({
      next: (response) => {
